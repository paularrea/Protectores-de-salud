...
const DetalleCitaDayFour = () => {
  const history = useHistory();
  const [patient, setCurrentPatient] = useState({});
  const [patientDate, setPatientDate] = useState();
  const { contextUser } = useContext(UserContext);
  const { id } = useParams();
  const interventions = contextUser && contextUser.agenda.day_3.list_of_events;
  ...
  useEffect(() => {
    const interventions =
      contextUser && contextUser.agenda.day_3.list_of_events;
    const currentAppointment =
      interventions &&
    ...
    setPatientDate(contextUser && contextUser.agenda.day_3.date_as_text);
  }, [contextUser, id]);
...
export default DetalleCitaDayFour;
