...
const DayOne = () => {
  ...
  const agendaDayOne =
    contextUser &&
    contextUser.agenda.day_2.list_of_events.map((intervention, id) => {
      return (
        <Link
          key={id}
          to={`intervention-details-3/${intervention.agenda_event_id}`}
        >
  ...
  const date = contextUser && contextUser.agenda.day_2.date_as_text;
